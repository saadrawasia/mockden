...
export const createSchema = async ({
  ...
  if (!name) return res.status(400).json({ message: 'Missing schema name' });
...
export const getSchema = async ({ id, res }: GetSchemaProps) => {
  try {
...
export const getAllSchemas = async (res: Response) => {
  try {
