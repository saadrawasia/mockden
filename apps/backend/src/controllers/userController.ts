...
import { deleteUser, getUserByClerkId, updatePassword, updateUser } from '@backend/services/userService';
...
export async function getUserRequest(req: Request, res: Response) {
  try {
    const { userId } = getAuth(req);
    if (!userId)
      return res.status(401).json({ message: 'Unauthorized Request' }); ;
