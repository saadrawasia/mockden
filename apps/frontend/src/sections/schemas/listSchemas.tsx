					...
					<Card key={schema.id} className="w-full gap-4">
						<CardHeader>
							<div className="flex flex-col gap-4">
								<CardTitle>
									<TypographyH5>/{schema.slug}</TypographyH5>
								</CardTitle>
							</div>
							...
							<div className="flex flex-col gap-4">
								<div className="flex flex-col gap-2">
									<TypographyP className="font-semibold">
										API: <span className="text-muted-foreground">(GET, POST, PUT, DELETE)</span>
									</TypographyP>
									<div
									...
									>
										<TypographyP className="text-muted-foreground">
											{config.BACKEND_URL}
											/mockdata/
											{project.slug}/{schema.slug}
										</TypographyP>
										<Copy
											className="hidden text-muted-foreground group-hover:flex md:hidden"
											size={20}
										/>
									</div>
								</div>
								...
								<div className="flex flex-col gap-2">
									...
									<div
										className="group flex w-fit cursor-pointer gap-2"
										onClick={() => copyToClipboard(`x-mockden-header: ${project.apiKey}`)}
									>
										<TypographyP className="text-muted-foreground">
											x-mockden-header: {project.apiKey}
										</TypographyP>
										<Copy
											className="hidden text-muted-foreground group-hover:flex md:hidden"
											size={20}
								...
								<div className="flex space-x-2">
									...
									/>
									<Label htmlFor="active" className="font-semibold text-md">
										Active
									</Label>
								</div>
							</div>
			...
			<AlertDialog open={openAlert} onOpenChange={setOpenAlert}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This action cannot be undone. This will permanently delete your Schema with all its
							records.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
